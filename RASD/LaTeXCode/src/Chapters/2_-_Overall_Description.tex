...
When they insert a new charging point that belongs to a new charging station, they create it, too.
...
When eMALL retrieves information about the charging stations available in the local area, it also retrieves all the extra info about the available plugs and power supplies.
...
In the email, eMALL also specifies the serial number associated with the charging point of the charging station where the EVD has to charge his EV\@.

\textbf{The EVD receives charging alerts about where to charge his EV} \\
eMALL offers smart functions about when the EVD might book a charge for his EV\@.
Hence, when an EVD inserts a new activity in his calendar, eMALL computes the best route to reach the destination through an external navigator API\@.
eMALL also checks the battery status of the EV, so it notifies the best itinerary for the EVD\@.
If the battery state doesn't allow the EVD to reach the destination place, then eMALL shows him the best route with the charging stations available along the road.

eMALL tries to minimize the costs.
...
If the EV can reach the destination, eMALL marks the route returned by the API as preferred.
On the other hand, eMALL finds charging stations along the road and selects the one with the minimum costs because it knows the details about the EV, for instance, the plug type.
...
In general, a CPO might have new charging stations to register in eMALL, and, as well, it might have charging points too.
...
Just like the CPO inserts new information about its product, it can also delete charging points or charging stations from eMALL\@.
